...
export const toggleStatus = createAsyncThunk(
    ...
    async function (id, {rejectWithValue, dispatch, getState}) {
        ...
        try {
            ...
            dispatch(toggleComplete({id}))// для серверного варианта
        } catch (error) {
            return rejectWithValue(error.message)
        }
    }
    ...
    reducers: {
        ...
        toggleComplete(state, action) {
           ...
           /!* if(userId.completed) {
            ...
            // Вариант для серверного
            const statusTodo = state.todos.find(todo => todo.id === action.payload.id)
            statusTodo.completed = !statusTodo.completed
